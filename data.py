...
def load_data(config, tokenizer):
    ...
    if os.path.exists(dataset_path):
        ...
        dataset = load_dataset("json", data_files=files, split="train",num_proc=30)

    else:
        dataset = load_dataset(dataset_path, split="train",num_proc=30, revision=config["revision"] if "revision" in config else None)
